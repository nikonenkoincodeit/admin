<template>
  ...
  >
    ...
    <template #cell(summary.total_amount)="{ rowData }"> {{ rowData.summary.total_amount }} ₴ </template>

    <template #cell(payment.status)="{ rowData }">
      <VaBadge
...
const columns = defineVaDataTableColumns([
  { label: 'Order №', key: 'order_id', sortable: true, sortingFn: customSorting },
  ...
  { label: 'Total Amount', key: 'summary.total_amount', sortable: true },
  { label: 'Delivery Address', key: 'delivery.address', sortable: false },
  { label: 'Payment', key: 'payment.status', sortable: true },
])
...
const props = defineProps({
  ...
  sortBy: { type: [String, Boolean] as PropType<string | false>, required: true },
...
function customSorting  (items: Order[], sortBy: string, sortingOrder: 'asc' | 'desc' | null)  {
...
console.log(1111);
  ...
  return sortingOrder === 'desc' ? sorted.reverse() : sorted
}
